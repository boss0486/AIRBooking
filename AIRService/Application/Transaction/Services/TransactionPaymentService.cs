...
namespace WebCore.Services
{
    ...
    public class TransactionPaymentService : EntityService<TransactionPayment>, ITransactionPaymentService
    {
        ...
        public ActionResult DataList(SearchModel model)
        {
            ...
            //
            var dtList = _connection.Query<TransactionPaymentResult>(sqlQuery, new { Query = Helper.Page.Library.FormatToUni2NONE(query) }).ToList();
            if (dtList.Count == 0)
        ...
        public ActionResult Update(TransactionPaymentUpdateModel model)
        {
            ...
            using (var _transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    TransactionPayment.ReceivedDate = Helper.Time.TimeHelper.FormatToDateSQL(receivedDate);
