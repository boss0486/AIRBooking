...
//            //
//            var dtList = _connection.Query<TransactionUserSpendingResult>(sqlQuery, new { Query = Helper.Page.Library.FormatNameToUni2NONE(query) }).ToList();
//            if (dtList.Count == 0)
...
//                    //    return Notifization.Error("Không thể cập nhật giao dịch");
//                    ////
//                    //var changeBalanceForUser = WalletService.ChangeBalanceForUser(new WalletTiketingChangeModel { ClientID = senderId, UserID = receivedUserId, Amount = amount, TransactionType = (int)TransactionEnum.TransactionType.IN }, dbConnection: _connection, dbTransaction: _transaction);
