...
namespace WebCore.Services
{
    ...
    public class WalletSpendingHistoryService : EntityService<WalletSpendingHistory>, IWalletSpendingHistoryService
    {
        ...
        public ActionResult DataList(SearchModel model)
        {
            ...
            string sqlQuery = @"SELECT * FROM App_WalletSpendingHistory WHERE dbo.Uni2NONE(Title) LIKE N'%'+ @Query +'%' " + whereCondition + " ORDER BY [CreatedDate] DESC";
            var dtList = _connection.Query<WalletSpendingHistory>(sqlQuery, new { Query = Helper.Page.Library.FormatNameToUni2NONE(query) }).ToList();
            if (dtList.Count == 0)
        ...
        public TransactionHistoryMessageModel WalletSpendingHistoryCreate(WalletSpendingHistoryCreateModel model,IDbConnection dbConnection, IDbTransaction dbTransaction = null)
        {
            ...
            //
            string customerId = model.AgentID;
