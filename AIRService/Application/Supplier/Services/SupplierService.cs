...
namespace WebCore.Services
{
    ...
    public class SupplierService : EntityService<Supplier>, ISupplierService
    {
        ...
        public ActionResult DataList(SearchModel model)
        {
            ...
            var dtList = _connection.Query<SupplierResult>(sqlQuery, new { Query = Helper.Page.Library.FormatNameToUni2NONE(query), TypeID = typeId }).ToList();
            if (dtList.Count == 0)
        ...
        public static string GetSupplierID(string id)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(id))
