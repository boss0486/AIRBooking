...
namespace WebCore.Services
{
    ...
    public class MenuControllerService : EntityService<MenuController>, IMenuControllerService
    {
        ...
        public ActionResult MenuControlSync(string routeArea)
        {
            // read json
            List<MvcControllerModel> listController = new List<MvcControllerModel>();
            string file = HttpContext.Current.Server.MapPath(@"/Areas/Management/Data/permission-data.json");
            ...
            using (var _transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var lstControllerIdInDb = menuControllerService.GetAlls(m => m.RouteArea.Equals(routeArea), transaction: _transaction).Select(m => m.ID).ToList();
                    ...
                    foreach (var controller in listController)
                    {
                        ...
                        var menuController = menuControllerService.GetAlls(m => !string.IsNullOrWhiteSpace(m.ID) && m.ID.Equals(controllerId), transaction: _transaction).FirstOrDefault();
                        if (menuController == null)
                        {
                            ...
                            foreach (var action in actionList)
                            {
                                ...
                                m.CategoryID.Equals(controllerId) &&
                                m.KeyID.Equals(actKeyId), transaction: _transaction).FirstOrDefault();
                                if (menuAction == null)
                                {
                        ...
                        else // update action 
                        {
                            ...
                            foreach (var action in actionList)
                            {
                                ...
                                var actTitle = action.Title;
                                string actionId = Helper.Security.Library.FakeGuidID(controllerId + actKeyId);
                                var menuAction = menuActionService.GetAlls(m => !string.IsNullOrWhiteSpace(m.ID) && m.CategoryID.Equals(controllerId) && m.KeyID.Equals(actKeyId), transaction: _transaction).FirstOrDefault();
                                if (menuAction == null)
                                {
                                    menuActionService.Create<string>(new MenuAction()
                                    {
                ...
                using (var service = new MenuControllerService())
                {
                    ...
                    if (dtList.Count > 0)
                    {
                        ...
                        foreach (var item in dtList)
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(id) && !string.IsNullOrWhiteSpace(item.ID) && item.ID.Equals(id))
                                select = "selected";
                            //
