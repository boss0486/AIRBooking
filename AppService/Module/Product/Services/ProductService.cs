...
namespace WebCore.Services
{
    ...
    public class ProductService : EntityService<Product>, IProductService
    {
        ...
        public ActionResult Datalist(ProductSearchModel model)
        {
            ...
            if (dtList.Count <= 0)
                return Notifization.NotFound(NotifizationText.NotFound + sqlQuery);
        ...
        public ActionResult Update(ProductUpdateModel model)
        {
            ...
            using (var transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var product = productService.GetAlls(m => !string.IsNullOrWhiteSpace(m.ID) && m.ID.ToLower().Equals(Id), transaction: transaction).FirstOrDefault();
                    if (product == null)
                        return Notifization.NotFound(NotifizationText.NotFound);
                    ...
                    product.LanguageID = Current.LanguageID;
        ...
        //##############################################################################################################################################################################################################################################################
        public ActionResult Details(ProductIDModel model)
        {
            if (model == null)
                return Notifization.Invalid();
            //
            ...
            if (lstPhoto.Count > 0)
                item.Photos = lstPhoto;
            return Notifization.DATALIST(NotifizationText.Success, data: item, role: null, paging: null);
        ...
        public List<ProductOption> DataOption(string langID)
        {
            try
            {
                string sqlQuery = @"SELECT * FROM View_App_Product_Option ORDER BY Title ASC";
