...
namespace WebCore.Services
{
    ...
    public class CMSUserSettingService : EntityService<CMSUserSetting>, ICMSUserSettingService
    {
        ...
        public ActionResult SendOtp(CMSUserEmailModel model)
        {
            ...
            string strToken = Helper.Security.Token.Create(cmsUserInfo.UserID, strGuid);
        ...
        public ActionResult ResetPassword(ResetPasswordModel model)
        {
            ...
            cmsUser.PassID = Helper.Security.Encryption256Hashed(model.Password);
            cmsUserLoginService.Update(cmsUser);
            return Notifization.Success(NotifizationText.UPDATE_SUCCESS, "/authorization");
        }
        ...
        public List<CMSUserOption> DataOption(string langID)
        {
            try
            {
                string sqlQuery = @"SELECT UserID AS ID, CONCAT(FirstName,LastName) as Title  
                                    FROM View_CMSUser_Option WHERE  IsBlock = 0 AND Enabled = 1 ORDER BY Title ASC";
                return _connection.Query<CMSUserOption>(sqlQuery, new { LangID = langID }).ToList();
            }
            catch
            {
                return new List<CMSUserOption>();
            }
        }
        //##############################################################################################################################################################################################################################################################
