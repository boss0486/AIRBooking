var pageIndex = 1;
var URLC = "/Management/Airline/Action";
var URLA = "/Management/Airline";
var AirlineController = {
    init: function () {
        AirlineController.registerEvent();
    },
    registerEvent: function () {
        $('#btnCreate').off('click').on('click', function () {
            ...
            //
            if (txtCode === '') {
                $('#lblCode').html('Không được để trống mã hàng không');
                flg = false;
            }
            else if (txtCode.length < 1 || txtCode.length > 5) {
                $('#lblCode').html('Mã hàng không giới hạn [1-5] ký tự');
                flg = false;
            }
            else if (!FormatKeyId.test(txtCode)) {
                $('#lblCode').html('Mã hàng không không hợp lệ');
                flg = false;
            }
            //
            if (txtTitle === '') {
                    ...
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (id.length > 0)
                                id = id.trim();
                            //  role
                            var action = HelperModel.RolePermission(result.role, "AirlineController", id);
                            //
                        ...
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, AirlineController.DataList);
                        }
                        return;
                    }
                    else {
