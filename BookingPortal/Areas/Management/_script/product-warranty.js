var pageIndex = 1;
var URLC = "/ProductWarranty";
var URLA = "/backend/ProductWarranty";
var productWarrantyController = {
    init: function () {
        productWarrantyController.registerEvent();
    },
    registerEvent: function () {
        $('#btnCreate').off('click').on('click', function () {
            ...
            if (flg) {
                productWarrantyController.Create();
            }
            else {
        ...
        $('#btnSearch').off('click').on('click', function () {
            productWarrantyController.DataList(1);
        });
        $('#btnUpdate').off('click').on('click', function () {
            ...
            // submit
            if (flg) {
                productWarrantyController.Update();
            }
            else {
                Notifization.Error(MessageText.DATAMISSING);
            }
                    ...
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (role !== undefined && role !== null) {
                                ...
                                if (role.Delete)
                                    action += `<a onclick="productWarrantyController.ConfirmDelete('${id}')"><i class='fas fa-trash'></i>&nbsp;Delete</a>`;
                                if (role.Detail)
                        ...
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, productWarrantyController.DataList);
                        }
                        return;
                    }
                    else {
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response !== null) {
                    if (response.status === 200) {
                        Notifization.Success(response.message);
                        productWarrantyController.DataList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
    ...
    ConfirmDelete: function (id) {
        productWarrantyController.Delete(id);
    }
};
productWarrantyController.init();
