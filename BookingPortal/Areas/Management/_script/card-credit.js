var pageIndex = 1;
var URLC = "/Management/Card-Credit/Action";
var URLA = "/Management/CardCredit";
var CardCreditController = {
    init: function () {
        CardCreditController.registerEvent();
    },
    registerEvent: function () {
        ...
        $('#btnSearch').off('click').on('click', function () {
            CardCreditController.DataList(1);
        });
                    ...
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (role !== undefined && role !== null) {
                                ...
                                if (role.Delete)
                                    action += `<a onclick="CardCreditController.ConfirmDelete('${id}')"><i class='fas fa-trash'></i>&nbsp;Delete</a>`;
                                if (role.Details)
                        ...
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, CardCreditController.DataList);
                        }
                        return;
                    }
                    else {
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response !== null) {
                    if (response.status === 200) {
                        Notifization.Success(response.message);
                        CardCreditController.DataList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
