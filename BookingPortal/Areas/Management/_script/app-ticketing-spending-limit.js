var pageIndex = 1;
var URLC = "/Management/TiketingSpendingLimit/Action";
var URLA = "/Management/TiketingSpendingLimit";
var arrFile = [];
var TiketingSpendingLimitController = {
    init: function () {
        TiketingSpendingLimitController.registerEvent();
    },
    registerEvent: function () {
        $('#btnSearch').off('click').on('click', function () {
            TiketingSpendingLimitController.DataList(1);
        });
        $('#btnApply').off('click').on('click', function () {
            ...
            // submit form
            if (flg) {
                TiketingSpendingLimitController.Setting();
            }
            else {
        ...
        $('#btnPaySearch').off('click').on('click', function () {
            TiketingSpendingLimitController.PayList(1);
        });
    },
                ...
                if (result !== null) {
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            //  role
                            var action = HelperModel.RolePermission(result.role, "TiketingSpendingLimitController", id);
                            //
                        ...
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, TiketingSpendingLimitController.DataList);
                        }
                        return;
                    }
                    else {
            ...
            success: function (result) {
                ...
                if (result !== null) {
                    if (result.status === 200) {
                        ...
                        $('tbody#TblData').html(rowData);
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, TiketingSpendingLimitController.PayList);
                        }
                        return;
                    }
                    else {
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response !== null) {
                    if (response.status === 200) {
                        Notifization.Success(response.message);
                        TiketingSpendingLimitController.PayList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
    ...
    ConfirmPayment: function (param) {
        Confirm.ConfirmYN(param, TiketingSpendingLimitController.ExpPayment, Confirm.Text_payment_spendinglimit);
    }
};
//
TiketingSpendingLimitController.init();
...
$(document).on("click", ".btn-payment", function () {
    ...
    TiketingSpendingLimitController.ConfirmPayment(model);
});