var pageIndex = 1;
var URLC = "/Management/Customer-Type/Action";
var URLA = "/Management/CustomerType";
var CustomerTypeController = {
    init: function () {
        CustomerTypeController.registerEvent();
    },
    registerEvent: function () {
        ...
        $('#btnSearch').off('click').on('click', function () {
            CustomerTypeController.DataList(1);
        });
                    ...
                    if (result.status === 200) {
                        ...
                        $.each(result.data, function (index, item) {
                            ...
                            if (role !== undefined && role !== null) {
                                ...
                                if (role.Delete)
                                    action += `<a onclick="CustomerTypeController.ConfirmDelete('${id}')"><i class='fas fa-trash'></i>&nbsp;Delete</a>`;
                                if (role.Details)
                        ...
                        if (parseInt(totalPage) > 1) {
                            Paging.Pagination("#Pagination", totalPage, currentPage, CustomerTypeController.DataList);
                        }
                        return;
                    }
                    else {
        ...
        AjaxFrom.POST({
            ...
            success: function (response) {
                if (response !== null) {
                    if (response.status === 200) {
                        Notifization.Success(response.message);
                        CustomerTypeController.DataList(pageIndex);
                        return;
                    }
                    else {
                        Notifization.Error(response.message);
